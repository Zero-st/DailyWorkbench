...
> **零构建 · 依赖极简且可撤回 · 本地起服务即可跑（离线优先）**

拆成大白话（信条经 [`docs/adr/0010`](docs/adr/0010-relax-zero-dependency-north-star.md) 修订，原为"零构建·零依赖·离线可跑"）：

- **零构建**——代码写完直接就能跑，没有"先 build 一下"这一步。别的前端项目常常改一行字也得先 `npm run build` 编译半天；这里不用，源码就是成品。好比**手写笔记本 vs 需要冲印的胶卷**：写完就能看，不用送去冲洗。（引入**构建步骤**仍是单向门，要写 ADR 才动。）
- **依赖极简且可撤回**——不装框架（没有 React/Vue）、不引打包工具，前端跑浏览器**原生 ES Modules**。要用第三方库也行，但只用**钉死版本、随仓入库、随时能撕掉换回手写**的那种（如 `vendor/marked.min.js`），且缺了它 App 核心也不崩。少一样必需的外部零件，就少一个"三年后它自己坏掉"的风险。
- **本地起服务即可跑（离线优先）**——真实入口是 `python -m backend.server` 起本地 http；靠 Service Worker（"离线随身缓存包"）把页面存本地，起过一次后断网也能打开看上次的数据。（注意：**直接双击 `index.html`（`file://`）在 Chrome 里通常打不开完整页面**——`app.js` 是 ES Module，浏览器不允许从 `file://` 加载它；起个本地服务才稳。）

为什么这么拧巴地追求"简单"？因为这是**一个人维护**的项目。单人项目最大的死法不是"功能不够"，而是"某天它突然跑不起来了"。所以宁可朴素，也要保证 `git clone` 下来、起个本地服务就能跑。

前端之外，另配一个**可选的本地 Python 服务**（`backend/server.py`，核心只用标准库；第三方依赖仅可选、缺了核心链路也能跑），给你补上「一键刷新 / AI 代理 / Obsidian 知识库」这些需要"联机干活"的能力。不想要？不开它，纯网页照样跑。
...
**最快**：把仓库下下来，起个本地静态服务器（**别直接双击 `index.html`**——`app.js` 是 ES Module，Chrome 不允许从 `file://` 加载，双击通常只出个残壳）：
...
此时用的是内置示例数据，够你先看个样子。想接真实数据、或测试"装到主屏"，见下面第四节。
...
- **怎么做**：`python -m http.server 8080` 后开 `http://localhost:8080`（**别双击 `index.html`**——ESM 在 `file://` 下加载受限，Chrome 里通常出残壳）。
