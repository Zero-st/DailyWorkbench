...
4. 否则按**后缀词表**（§2）落对应子目录：讲「怎么做」→`guides/`；讲「为什么/怎么设计」→`design/`；「选型/调研」→`research/`；「方向/进度/复盘」→`planning/`；「schema/产物」→`reference/`。
...
| [reference/devhtml/](reference/devhtml/) | reference | 生成的 html 产物（产品评分卡等） |
