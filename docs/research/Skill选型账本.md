...
| **mattpocock/skills** | 框架（skill 合集，非单一强制流程） | [mattpocock/skills](https://github.com/mattpocock/skills) | **264.7k**（fork 22.1k，fork 数为 09-15 快照） | 2026-09-17 推送（2026-09-18 `gh api` 实测快照，晚于本节其余条目） | 社区头部；作者 Matt Pocock（知名 TS/AI 教育者） | 轻（可单个挑装） | 想要**可拆开单独装的一批工程纪律**（TDD 按改动量分档、疑难 bug 诊断循环、需求追问、merge 冲突处理等），而不是一整套必须遵守的强制流程 |

**和 Superpowers / Spec-Kit / BMAD 的关键差异**：后三者是**强制流程**（本节已有判据：三选一，同时装会互相打架）；mattpocock/skills 明确**反对**这种"重流程接管你的过程"的设计，自述定位就是这类框架的对照组——**小、可拆、可改、模型无关**，可以只装其中一两个（比如只要 `tdd` 或只要 `diagnosing-bugs`），不需要囫囵吞下整套。所以它**不适用**上面"强制流程三选一"的判据，是独立于三者之外的第四种选择方式。本项目已按需引入其中 14 个（2026-09-18 新增 `improve-codebase-architecture` + 它依赖的 `codebase-design`、`domain-modeling`；三者需成组装，单装 `improve-codebase-architecture` 会在追问阶段调不到兄弟技能），具体清单和触发方式见姊妹篇 [`工具链-MCP与Skill地图.md`](工具链-MCP与Skill地图.md)（此处只记事实存在，不复制清单）。
...
- 2026-09-18 · v1.8 · **mattpocock/skills 再引 3 个 + 记一条「改造后再装」的先例**：引入 `improve-codebase-architecture`（全仓架构扫描→HTML 报告→逐个追问）及其依赖 `codebase-design`（深模块词汇表）、`domain-modeling`（CONTEXT.md 术语表 + ADR），全部中文化，11→14 个。**两处必须改造才能装**：① 原技能把报告写到系统 `$TMPDIR`/`/tmp`，改为仓内 `docs/reference/devhtml/architecture-review/`（按日期命名，随仓留档）；② 原 `ADR-FORMAT.md` 自带一套 ADR 模板，与宪章「附·ADR 轻量机制」冲突，改为只做指向、不另起模板。**未装** `grill-with-docs`——实测原文根本没有这个技能名，是 WebFetch 摘要臆造的，真实依赖是 `grilling`（已有）；教训：装前用 `curl` 拉原文核对，别信二手摘要。同日另引入 `vercel-labs/skills` 的 `find-skills`（技能发现/安装元技能），并核实 skills.sh 是 Vercel 第三方装机量榜、非官方权威源，官方参考实现仍是 `anthropics/skills`。★ 264.7k 为 2026-09-18 `gh api` 实测。
